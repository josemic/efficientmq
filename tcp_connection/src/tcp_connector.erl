...
handle_info(timeout, #state{port=Port,count=Count} = State) ->
    {ok, ListenSocket} = gen_tcp:listen(Port, [binary, {active, once}, {reuseaddr,true}, {packet,4}, {nodelay,true}, {exit_on_close,false}]),
...
wait_for_accept_loop(ListenSocket, MaxNumberOfConcurrentConnections, Instance, State) ->
    ...
    error_logger:info_msg("ts_root_sup:start_connection_worker: Root_Sup_PID(~w), Socket(~w), Instance(~w), Result(~w)~n", [ts_root_sup, Socket, Instance, Connection_Result]),
